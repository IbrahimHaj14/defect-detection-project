"""MLflow helpers using the repository's shared local tracking store."""
...
def start_c2_run(
    ...
    """Start a C2 MLflow run in C1's existing file-backed store."""
    ...
    with mlflow.start_run(run_name=run_name) as active_run:
        if params:
            mlflow.log_params({key: _normalise_param(value) for key, value in params.items()})
        yield active_run
